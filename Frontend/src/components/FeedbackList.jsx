...
const FeedbackList = ({ feedbacks, projectId }) => {
  const handleDelete = async (feedbackId) => {
    try {
      await deleteDoc(doc(db, 'projects', projectId, 'feedbacks', feedbackId));
  ...
  <div key={feedback.id} className="border-b pb-2">
    <p className="text-sm text-gray-600">Date: {feedback.createdAt && new Date(feedback.createdAt.toDate()).toLocaleString()}</p>
